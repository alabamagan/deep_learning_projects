...
from pytorch_med_imaging.networks.layers import ResidualBlock3d, Conv3d, InvertedConv3d, DoubleConv3d
from pytorch_med_imaging.networks.layers.StandardLayers3D import MaskedSequential3d

__all__ = ['AttentionModule_25d', 'AttentionModule_25d_recur']
...
class SoftMaskBranch_25d(nn.Module):
    r"""Soft-mask branch of the Residual Attention Network adapted for 2.5D (3D kernels, axial stride=1).
    ...
    Args:
        ...
        stage (int):
            Depth of the encoder: 0 → 3 skip levels, 1 → 2, 2 → 1.
        mask (bool, Optional):
            Use :class:`MaskedSequential3d` for the output conv. Default ``False``.
    """
    def __init__(self, in_ch: int, out_ch: int, r: int = 1, stage: int = 0,
                 mask: Optional[bool] = False) -> None:
        ...
        self.skip0 = nn.Sequential(*(
            ...
            [ResidualBlock3d(out_ch, out_ch) for _ in range(r - 1)]
        ))

        for i in range(skipconnection):
            if i != 0:
                self.add_module(f'skip{i}', nn.Sequential(*[
                    ResidualBlock3d(out_ch, out_ch) for _ in range(r)
                ]))
            self.add_module(f'down{i}', nn.Sequential(*(
                ...
                [ResidualBlock3d(out_ch, out_ch)
                 for _ in range(r * 2 if i == skipconnection - 1 else r)]
            )))
...
class AttentionModule_25d(nn.Module):
    r"""Residual attention module for 2.5D feature maps.

    Combines a trunk branch (residual blocks) with a soft-mask branch and adds
    the element-wise attention back to the trunk: ``trunk * (1 + mask)``.

    Args:
        in_ch (int): Input channels.
        out_ch (int): Output channels.
        stage (int): Passed to :class:`SoftMaskBranch_25d` to control encoder depth.
        p (int): Number of residual blocks before/after the attention sub-modules.
        t (int): Number of residual blocks in the trunk branch.
        r (int): Number of residual blocks per level inside the soft-mask branch.
        save_mask (bool): Cache the attention mask for inspection. Default ``False``.
    """
    def __init__(self, in_ch, out_ch, stage, p=1, t=2, r=1, save_mask=False):
        super(AttentionModule_25d, self).__init__()

        self.in_conv = nn.Sequential(*[ResidualBlock3d(in_ch, out_ch) for _ in range(p)])
        self.soft_mask_branch = SoftMaskBranch_25d(out_ch, out_ch, r, stage=stage)
        self.trunk_branch = nn.Sequential(*[ResidualBlock3d(out_ch, out_ch) for _ in range(t)])
        self.out_conv = nn.Sequential(*[ResidualBlock3d(out_ch, out_ch) for _ in range(p)])
...
class AttentionModule_25d_recur(nn.Module):
    r"""Variant of :class:`AttentionModule_25d` that uses a recurrent soft-mask branch.
    ...
    def __init__(self, in_ch, out_ch, p=1, t=2, r=1, save_mask=False):
        ...
        self.in_conv = nn.Sequential(*[ResidualBlock3d(in_ch, out_ch)] * p)
        self.soft_mask_branch = SoftMaskBranch_25d_recur(out_ch, out_ch, r)
        self.trunk_branch = nn.Sequential(*[ResidualBlock3d(out_ch, out_ch)] * t)
        self.out_conv = nn.Sequential(*[ResidualBlock3d(out_ch, out_ch)] * p)
